...
/*   Updated: 2024/05/25 09:31:22 by glemaire         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_lstadd_str(t_data *data, char *name, int n)
{
	...
	while (name[i])
	{
		...
		if (!new_stru)
			reloop(data, EXIT_FAILURE, "new_stru", strerror(ENOMEM));
		if (name[i] == ' ')
...
static char	*create_name(t_data *data, t_list *cursor)
{
	...
	if (!name)
		reloop(data, EXIT_FAILURE, "name", strerror(ENOMEM));
	i = 0;
	nextnode = cursor->next;
	while (i < len)
	{
...
static void	add_exp(t_data *data, char *name, int len, int i)
{
	...
	while (data->envp[j])
	{
			...
			data->envp[j][len] == '=' && !name[len])
		{
			...
			if (!exp)
				reloop(data, EXIT_FAILURE, "exp", strerror(ENOMEM));
		}
		j++;
	}
	free(name);
